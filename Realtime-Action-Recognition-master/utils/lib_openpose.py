...
def _get_input_img_size_from_string(image_size_str):
    ...
    if width % 16 != 0 or height % 16 != 0:
        raise Exception(
            'Width and height should be multiples of 16. w=%d, h=%d' % (width, height))
...
class SkeletonDetector(object):
    ...
    def __init__(self, model="cmu", image_size="432x368"):
        ...
        self._model = model
        # Resize heatmaps before they are post-processed. If image_size is small, this should be large.
    ...
    def draw(self, img_disp, humans, draw_pta=False):
        ''' Draw human skeleton on img_disp inplace.
        Argument:
            img_disp {RGB image}
            humans {a class returned by self.detect}
        '''
        img_w = img_disp.shape[1]
        img_h = img_disp.shape[0]
        img_disp = TfPoseEstimator.draw_humans(img_disp, humans, imgcopy=False)
        # Realtime-Action-Recognition-master/src/githubs/tf-pose-estimation/tf_pose/estimator.py: class TfPoseExtimator: def draw_humans(npimg, humans, imgcopy=False):
        ...
        if draw_pta:
            ...
            for human in humans:                
                ...
                if face_box_new is not None:
                    cv2.rectangle(
                        img_disp,
                        face_box_new[0], face_box_new[3],
                        (0, 255, 0), 2)
                # Draw body box
                if body_box_new is not None:
                    cv2.rectangle(
                        img_disp,
                ...
                #     cv2.circle(img_disp, (int(r_andke_pt[0]), int(r_andke_pt[1])), 10, (0, 0, 255), -1)
                # if l_andke_pt is not None:
                #     cv2.circle(img_disp, (int(l_andke_pt[0]), int(l_andke_pt[1])), 10, (0, 0, 255), -1)

        if IS_DRAW_FPS:
            cv2.putText(img_disp,
                        "fps = {:.1f}".format(
                            (1.0 / (time.time() - self._prev_t))),
